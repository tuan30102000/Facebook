...
import { CloseIcon, LoadIcon } from './IconCustom/IconCustom';

CofirmBox.propTypes = {

};

function Btn({ text, bgColor = 'bg-[#dc3545]', onClick = null, isLoading = false }) {
    const handleClick = () => onClick && onClick()
    return (
        <>
            <button disabled={isLoading} onClick={handleClick} className={clsx('rounded-[3px] py-[10px] w-[120px] text-white opacity-90 hover:opacity-100 text-[16px]', bgColor)}>
                {isLoading ? <LoadIcon isLoading={isLoading} /> : <>{text}</>}
            </button>
        </>
    )
}
...
function CofirmBox({ highlightMessage, message, isLoading = false, onConfirm, confirmText = 'Delete', confirmBg = 'bg-[#F15E5E]', closeModal, }) {
    return (
        <div className='w-[400px] h-[380px] bg-white pt-10 rounded-[5px] px-[36px]'>
            <div
                className="rounded-crical border-solid border-[3px] w-20 h-20 flex justify-center items-center mx-auto border-red-500">
            ...
            <div className="flex justify-center gap-[10px] mt-[30px]">
                ...
                <Btn text={confirmText} isLoading={isLoading} onClick={onConfirm} bgColor={confirmBg} />
            </div>
