...
function UserInfoMini({ avatarUrl, displayName, userId = '', avtSize = '', isMargin = true }) {
    return (
        <Link to={'/profile/' + userId} className='flex w-max' >
            ...
            {displayName && <p className={clsx('text-[#050505] font-bold hover:underline', { 'ml-2': isMargin })}>{displayName}</p>}
        </Link>
    );
}
